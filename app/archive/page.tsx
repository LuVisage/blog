        ...
        <div className="space-y-8">
          {archive.map(({ year, months }, yearIdx) => (
            <section key={year} className="animate-slide-right" style={{ animationDelay: `${yearIdx * 100}ms` }}>
              {/* Year header */}
              <h2 className="text-2xl lg:text-3xl font-bold text-gray-500 dark:text-gray-400 mb-4 sticky top-20 z-10">
                {year}
              </h2>
                      ...
                      {posts.map((post) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {post.category && (
                              <span className="hidden sm:inline-flex items-center px-2 py-0.5 rounded-full text-xs bg-white/10 dark:bg-white/5 text-gray-500 dark:text-gray-400 border border-white/10 dark:border-white/10">
                                {post.category}
                              </span>
                            )}
                          </Link>
