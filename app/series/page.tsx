      ...
      {series.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          {series.map(({ series: name, count }) => (
            ...
            >
              ...
              <div className="mt-3 flex items-center gap-1 text-xs text-pink-500 dark:text-pink-400 group-hover:gap-2 transition-all">
                <span>查看系列</span>
        ...
        <div className="text-center py-20 rounded-3xl glass-card">
          ...
          <p className="text-gray-300 dark:text-gray-600 text-sm mt-2">
            在文章 frontmatter 中添加 <code className="px-1.5 py-0.5 rounded bg-pink-50 dark:bg-purple-950/50 text-pink-500 text-xs">series</code> 和 <code className="px-1.5 py-0.5 rounded bg-pink-50 dark:bg-purple-950/50 text-pink-500 text-xs">seriesOrder</code> 字段
          </p>
        </div>
      )}
    </div>
