...
export function ThemeToggle() {
  ...
  if (!mounted) return <button className="w-9 h-9 rounded-xl glass flex items-center justify-center cursor-pointer" aria-label="切换主题" />

  const isDark = theme === 'dark'
  return (
    <button ref={btnRef} onClick={toggleTheme} className="w-9 h-9 rounded-xl glass flex items-center justify-center transition-all duration-200 cursor-pointer hover:border-[var(--color-border-strong)]" aria-label={isDark ? '切换到亮色模式' : '切换到暗色模式'}>
